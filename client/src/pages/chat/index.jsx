...
import EmptyChatContaienr from "./components/empty-chat-container";
        ...
        <div className="flex h-[100vh] text-white overflow-hidden poppins-medium">
            {
                isUploading && (<div className="h-[100vh] w-[100vw] fixed top-0 z-10 left-0 bg-black/80 flex items-center justify-center flex-col gap-5 backdrop-blur-lg">
                    <h5 className="text-lg sm:text-5xl animate-pulse">Uploading File</h5>
                    {fileUploadProgress}%
                </div>
            )}
            {
                isDownloading && (<div className="h-[100vh] w-[100vw] fixed top-0 z-10 left-0 bg-black/80 flex items-center justify-center flex-col gap-5 backdrop-blur-lg">
                    <h5 className="text-lg sm:text-5xl animate-pulse">Downloading File</h5>
                    {fileDownloadProgress}%
                </div>
            )}
            <ContactsContainer />
            {
                selectedChatType === undefined ? (<EmptyChatContaienr />) : (<ChatContainer />)
            }
        </div>
    );
};
